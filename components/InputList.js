		...
		<div className="flex flex-col">
			{value.map((option, i) => (
				<div key={i} className="flex flex-row mt-2">
					<Input onChange={(e) => onChangeInput(i, e.target && e.target.value)} name={`${name}[${i}]`}
						defaultValue={option.value} error={error || option.error} rounded="rounded rounded-r-none"/>
					<Button color="red" px="4" rounded="rounded rounded-l-none" disabled={count <= 1}
